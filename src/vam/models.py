...
class VAM(nn.Module):
    ...
    def __call__(self, stimuli, rts, responses, g, key, training: bool):
        drift_mean = self.get_drifts(stimuli, training=training)
        elbo = self.get_elbo(rts, responses, drift_mean, g, key)
...
class LBAVI(nn.Module):
    ...
    @property
    def mu(self):
        return jnp.concatenate([self.c, self.a, self.t0, self.gamma])
    ...
    def setup(self):
        ...
        self.gamma = self.param("gamma", constant_init(config.gamma_init), (1,))
    ...
    def elbo(self, rts, choices, drifts, g, key):
        if self.config.elbo_type == "standard":
            raise NotImplementedError("Standard ELBO not updated for 4 params yet")
        elif self.config.elbo_type == "local":
            get_jac = vmap(vmap(lba_jac_adj, in_axes=(0, 0, 0, 0)), in_axes=(0, 0, 0, 0))
            get_logp = jax.vmap(
                jax.vmap(lba_logp, in_axes=[None, None, None, None, 0, 0, 0, 0, None]),
        ...
        gamma = jax.nn.sigmoid(rsample[..., 3])
        # gamma = jnp.ones_like(gamma)
        b = c + a

        # Reconstruction error term
        log_px_z = get_logp(rts, choices, drifts, g, b, a, t0, gamma, self.config.s).squeeze()
        ...
        jac_adj = get_jac(
            ...
            rsample[..., 0],
            rsample[..., 2],
            jax.nn.log_sigmoid(rsample[..., 3]) + jax.nn.log_sigmoid(-rsample[..., 3]),
        )
    ...
    def __call__(self, rts, choices, drifts, g, key):
        return self.elbo(rts, choices, drifts, g, key)
...
class CNN(nn.Module):
    ...
    def __call__(self, x, training: bool):
        # Convolutional layers from config
        for n in self.config.conv_n_features:
            x = nn.Conv(features=n, kernel_size=(3, 3), padding="SAME", dtype=self.param_dtype)(x)
            x = nn.relu(x)
        ...
        # Fully connected layers from config
        for n in self.config.fc_n_units:
            x = nn.Dense(features=n, dtype=self.param_dtype)(x)
            x = nn.relu(x)
            x = nn.Dropout(rate=self.config.dropout_rate, deterministic=not training)(x)
            # self.sow("intermediates", "features", x) # If sow was used

        # Final output layer
        x = nn.Dense(features=self.config.n_acc, dtype=self.param_dtype)(x) # Use 2 instead of self.config.n_acc
