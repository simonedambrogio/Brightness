...
class TaskData:
    """
    Container for cognitive task data (limited to the Risky Gamble task).
    ...
    def __init__(
        ...
        self.expt_name = config.expt_name # Experiment Name
    ...
    def _prep_data_slow(self):
        ...
        data["trial_idx"] = jnp.arange(len(data["rts"]))

        split_data = {}
        split_idx = self._get_split_idx(len(data["rts"]))
